...
@section('content')
    <div class="page-banner" style="background-image: url({{ asset('public/uploads/'.$g_setting->banner_job) }})">
        ...
        <div class="text">
            <h1>{{ JOB_TITLE }}: {{ $job_detail->job_title }}</h1>
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="career-detail">
                        ...
                        <div class="item">
                            <h3>{{ JOB_RESPONSIBILITY }}:</h3>
                            {!! clean($job_detail->job_responsibility) !!}
                        </div>

                        <div class="item">
                            <h3>{{ EDUCATIONAL_QUALIFICATION }}:</h3>
                            {!! clean($job_detail->job_education) !!}
                        </div>
                        <div class="item">
                            <h3>{{ EXPERIENCE_REQUIREMENT }}:</h3>
                            {!! clean($job_detail->job_experience) !!}
                        </div>
                        <div class="item">
                            <h3>{{ ADDITIONAL_REQUIREMENT }}:</h3>
                            {!! clean($job_detail->job_additional_requirement) !!}
                        </div>
                        <div class="item">
                            <h3>{{ OTHER_BENEFIT }}</h3>
                            {!! clean($job_detail->job_benefit) !!}
                        </div>
                        <div class="item">
