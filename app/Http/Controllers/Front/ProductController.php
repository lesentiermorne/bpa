...
class ProductController extends Controller
{
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if($product_qty > $product_detail->product_stock)  {
            return redirect()->back()->with('error', ERROR_MESSAGE_TOTAL_ITEMS_STOCK.$product_detail->product_stock);
        }
        ...
        if(session()->has('cart_product_id'))
        {
            ...
            if(in_array($product_id,$arr_cart_product_id)) {
                return redirect()->back()->with('error', ERROR_MESSAGE_PRODUCT_ALREADY_ADDED_TO_CART);
            }
        }
        ...
        return redirect()->back()->with('success', SUCCESS_MESSAGE_ITEM_ADDED_TO_CART);
    }
    ...
    public function cart_item_delete($id)
    {
        ...
        return redirect()->back()->with('success', SUCCESS_MESSAGE_ITEM_DELETED_FROM_CART);
    }
    ...
    public function update_cart(Request $request)
    {
        ...
        if($error_msg==1)
        {
            return redirect()->back()->with('error', ERROR_MESSAGE_QUANTITY_EXCEED_STOCK);
        }

        return redirect()->back()->with('success', SUCCESS_MESSAGE_CART_UPDATED);
    }
    ...
    public function shipping_update(Request $request)
    {
        ...
        return redirect()->back()->with('success', SUCCESS_MESSAGE_SHIPPING_METHOD_SELECT);
    }
    ...
    public function coupon_update(Request $request)
    {
		...
		if(!$coupon_detail) {
            return redirect()->back()->with('error', ERROR_MESSAGE_WRONG_COUPON_CODE);
        }
        ...
        if($coupon_detail->coupon_existing_use == $coupon_detail->coupon_maximum_use) {
            return redirect()->back()->with('error', ERROR_MESSAGE_COUPON_CODE_MAX_USED);
        }

        if($today < $coupon_detail->coupon_start_date) {
            return redirect()->back()->with('error', ERROR_MESSAGE_DATE_COUPON_CODE_APPLY_NOT_COME);
        }

        if($today > $coupon_detail->coupon_end_date) {
            return redirect()->back()->with('error', ERROR_MESSAGE_DATE_COUPON_CODE_EXPIRE);
        }
        ...
        return redirect()->back()->with('success', SUCCESS_MESSAGE_COUPON_SELECTED);
    }
