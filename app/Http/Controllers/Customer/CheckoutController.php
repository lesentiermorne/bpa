...
class CheckoutController extends Controller
{
    public function login(Request $request)
    {
        ...
        $request->validate(
            ...
            [
                'customer_email.required' => ERROR_MESSAGE_EMAIL_EMPTY,
                'customer_email.email' => ERROR_MESSAGE_EMAIL_VALID,
                'customer_password.required' => ERROR_MESSAGE_PASSWORD_EMPTY
            ]
        );
        ...
        if(!$check_email)
        {
        	return redirect()->back()->with('error', ERROR_MESSAGE_EMAIL_NOT_FOUND);
        }
        else
        {
        	...
        	{
        		return redirect()->back()->with('error', ERROR_MESSAGE_PASSWORD_WRONG);
        	}
        }

        if($check_email->customer_status!='Active') {
            return redirect()->back()->with('error', ERROR_MESSAGE_CUSTOMER_NOT_ACTIVE);
        }
        ...
        return redirect()->route('front.checkout')->with('success', SUCCESS_MESSAGE_LOGGED_IN_AS_CUSTOMER);;
    }
    ...
    public function billing_shipping(Request $request)
    {
        ...
        $request->validate(
            ...
            [
                'billing_name.required' => ERROR_MESSAGE_BILLING_NAME_EMPTY,
                'billing_email.required' => ERROR_MESSAGE_BILLING_EMAIL_EMPTY,
                'billing_email.email' => ERROR_MESSAGE_BILLING_EMAIL_VALID,
                'billing_phone.required' => ERROR_MESSAGE_BILLING_PHONE_EMPTY,
                'billing_country.required' => ERROR_MESSAGE_BILLING_COUNTRY_EMPTY,
                'billing_address.required' => ERROR_MESSAGE_BILLING_ADDRESS_EMPTY,
                'billing_state.required' => ERROR_MESSAGE_BILLING_STATE_EMPTY,
                'billing_city.required' => ERROR_MESSAGE_BILLING_CITY_EMPTY,
                'billing_zip.required' => ERROR_MESSAGE_BILLING_ZIP_EMPTY
            ]
        );
    ...
    public function stripe()
    {
        ...
        if(isset($_POST['stripeToken']))
        {
            ...
            for($i=0;$i<count($arr_cart_product_id);$i++)
            {
                ...
                $product_row .= '
                <b>'.PRODUCT_NO.($i+1).'</b><br>
                '.PRODUCT_NAME.': '.$product_detail->product_name.'<br>
                '.PRODUCT_PRICE.': $'.$product_detail->product_current_price.'<br>
                '.PRODUCT_QUANTITY.': '.$arr_cart_product_qty[$i].'<br>
                ';
            }
            ...
            $payment_method = '
            '.PAYMENT_METHOD.': '.STRIPE.'<br>
            '.CARD_LAST_4_DIGIT.': '.$response->payment_method_details->card->last4.'<br>
            '.EXPIRY_MONTH.': '.$response->payment_method_details->card->exp_month.'<br>
            '.EXPIRY_YEAR.': '.$response->payment_method_details->card->exp_year;
            ...
            return Redirect()->to('/')->with('success', SUCCESS_MESSAGE_PAYMENT);
        }
    }
